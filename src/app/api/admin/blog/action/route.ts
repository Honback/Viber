...
export async function POST(request: Request) {
  ...
  if (!slug || !action) {
    return createRedirectResponse(redirectTo, { error: "잘못된 요청입니다." });
  }
  ...
  try {
    ...
    if (action === "publish") {
      ...
      if (!pattern.test(source)) {
        return createRedirectResponse(redirectTo, { error: "해당 포스트를 찾을 수 없습니다." });
      }
      ...
      return createRedirectResponse(redirectTo, { error: "알 수 없는 액션입니다." });
    }
    ...
    return createRedirectResponse(redirectTo, { notice: messages[action] });
  } catch (err) {
    const message = err instanceof Error ? err.message : "알 수 없는 오류";
    return createRedirectResponse(redirectTo, { error: message });
  }
}
